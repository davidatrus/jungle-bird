...
export type TicketTypeSummary = {
  eventId: string;
  ticketTypeId: string;
  sanityTicketTypeId: string | null; // <-- NEW
  name: string;
  currency: string;
...
export async function getPrimaryTicketTypeSummaryBySanityEventId(
  ...
  // Primary = earliest created ticket type that HAS sanity_ticket_type_id
  const res = await pool.query(
    `
    select
      e.id as event_id,
      tt.id as ticket_type_id,
      tt.sanity_ticket_type_id,
      tt.name,
      tt.currency,
    ...
    where e.sanity_event_id = $1
      and tt.sanity_ticket_type_id is not null
  ...
  return {
    ...
    sanityTicketTypeId: r.sanity_ticket_type_id
      ? String(r.sanity_ticket_type_id)
      : null,
    name: String(r.name),
...
export async function getTicketTypeSummaryBySanityEventAndTicketType(
  sanityEventId: string,
  ...
  return {
    eventId: String(r.event_id),
    ticketTypeId: String(r.ticket_type_id),
    sanityTicketTypeId: String(r.sanity_ticket_type_id),
    name: String(r.name),
    currency: String(r.currency),
    unitAmountCents: Number(r.unit_amount_cents),
    capacity: Number(r.capacity),
