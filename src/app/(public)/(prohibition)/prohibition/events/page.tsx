...
export default async function ProhibitionEventsPage({
  searchParams,
}: {
  searchParams?: Promise<{ view?: string }>;
}) {
  const venueKey = 'prohibition' as const;

  const sp = (await searchParams) ?? {};
  const view = (sp.view as View) || 'upcoming';

  const events = await client
    .fetch<SanityEventListItem[]>(qEventsList, { venueKey })
    .catch(() => []);
    ...
    <main className="mx-auto w-full max-w-5xl px-4 py-12">
      <header className="mb-8">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          <Link
            href={{ pathname: '/prohibition/events', query: { view: 'past' } }}
            className={`rounded-full border px-4 py-2 text-xs font-semibold ${
              view === 'past'
                ? 'border-white/20 bg-white/10 text-white'
                : 'border-white/10 bg-white/5 text-white/70 hover:bg-white/10'
          ...
          <Link
            href={{ pathname: '/prohibition/events', query: { view: 'all' } }}
            className={`rounded-full border px-4 py-2 text-xs font-semibold ${
              view === 'all'
                ? 'border-white/20 bg-white/10 text-white'
                : 'border-white/10 bg-white/5 text-white/70 hover:bg-white/10'
