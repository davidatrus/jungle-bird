...
const VENUE_KEY = 'prohibition' as const;
const FALLBACK_OG = '/images/og/prohibition-og.webp';
...
export async function generateMetadata({
  ...
  const event = await client
    .fetch<SanityEvent | null>(qEventBySlug, { slug, venueKey: VENUE_KEY })
    .catch(() => null);

  const fallbackTitle = 'Prohibition Events | Prohibition Lounge Calgary';
  const fallbackDescription =
    'Discover upcoming speakeasy nights, cocktails, and special events at Prohibition Lounge Calgary.';
  ...
  if (!event) {
    return {
      ...
      alternates: {
        canonical: `/prohibition/events/${slug}`,
      },
      openGraph: {
        type: 'website',
        url: `https://www.junglebirdtikiyyc.com/prohibition/events/${slug}`,
        title: fallbackTitle,
        description: fallbackDescription,
        images: [
          {
  ...
  const title = `${event.title} | Prohibition Events`;
  const description =
    event.shortDescription ||
    'Join us at Prohibition Lounge Calgary for a vintage-inspired speakeasy event experience.';
  const canonical = `/prohibition/events/${event.slug}`;
...
export default async function ProhibitionEventDetailPage({
  params,
}: {
  params: Promise<Params>;
}) {
